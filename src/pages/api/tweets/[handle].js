...
export default async function handler(req,res){
  ...
  try{const rows=useApify?await fetchFromApify(handle,days):await fetchFromNitter(handle,days);rows.sort((a,b)=>a.timestamp_utc<b.timestamp_utc?1:-1);res.status(200).json(rows);}catch(e){res.status(500).json({error:String(e)});}}
