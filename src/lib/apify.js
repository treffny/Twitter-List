export async function fetchFromApify(handle, days=2) {
  ...
  for (let i=0; i<20; i++) {
    ...
    const runRes = await fetch(`https://api.apify.com/v2/actor-runs/${runId}?token=${process.env.APIFY_TOKEN}`);
    const run = await runRes.json();
    status = run?.data?.status;
    datasetId = run?.data?.defaultDatasetId || run?.data?.outputDatasetId;
  ...
  for (const it of items) {
    ...
    out.push({
      timestamp_utc: dt ? dt.toISOString() : new Date().toISOString(),
      author_handle: handle,
      tweet_id: id,
      tweet_url: `https://x.com/${handle}/status/${id}`,
      tweet_text: text,
      media_present: !!(it.media && it.media.length)
    });
  }
  return out;
}
